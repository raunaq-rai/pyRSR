...
try:
    from PyRSRX.cython_models import build_model_flam_linear_cython
    HAS_CYTHON = True
except ImportError:
    HAS_CYTHON = False
...
def build_model_flam_linear(params, lam_um, z, grating, which_lines, mu_seed_um):
    ...
    Uses Cython implementation if available.
    """
    nL = len(which_lines)
    
    # Ensure params are contiguous float64 for Cython
    params = np.asarray(params, dtype=np.float64, order='C')
    lam_um = np.asarray(lam_um, dtype=np.float64, order='C')

    if HAS_CYTHON:
        # Use Cython version
        return build_model_flam_linear_cython(params, lam_um, nL, which_lines)
    ...
    pix_A = np.median(np.diff(lam_A)) if lam_A.size > 1 else 1.0
